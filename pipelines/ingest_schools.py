...
from backend.app.models import Ward, School
...
logger = logging.getLogger("school_ingestion")
...
def ingest_schools(json_path: str):
    """
    Ingests educational institutions from raw OSM JSON into the database.
    Performs boundary filtering, validation, centroid normalization, and idempotent bulk upsert.
    """
    logger.info("School ingestion started...")
    ...
    try:
        ...
        # Query all existing school osm_ids
        existing_schools = db.query(School).all()
        existing_map = {int(s.osm_id): s for s in existing_schools if s.osm_id is not None}
        ...
        for elem in elements:
            ...
            try:
                ...
                if osm_id in existing_map:
                    school = existing_map[osm_id]
                    school.name = name
                    school.institution_type = institution_type
                    school.geom = from_shape(geom_shape, srid=4326)
                    updated += 1
                else:
                    new_school = School(
                        osm_id=osm_id,
                ...
                errors += 1
                logger.error(f"Error processing OSM school element {osm_id}: {fe}")
                continue
    ...
    logger.info("School ingestion completed.")

if __name__ == "__main__":
    raw_file = os.path.join(project_root, "data", "raw", "infrastructure", "bhubaneswar_schools.json")
    ingest_schools(raw_file)
