...
from backend.app.models import Ward, PoliceStation
...
logger = logging.getLogger("police_ingestion")
...
def ingest_police(json_path: str):
    """
    Ingests police stations from raw OSM JSON into the database.
    Performs boundary filtering, validation, centroid normalization, and idempotent bulk upsert.
    """
    logger.info("Police ingestion started...")
    ...
    police_elements = [elem for elem in elements if elem.get("tags", {}).get("amenity") == "police"]
    total_elements = len(police_elements)
    logger.info(f"Raw features matching amenity=police: {total_elements}")
    ...
    try:
        ...
        # Query all existing police station osm_ids
        existing_police = db.query(PoliceStation).all()
        existing_map = {int(p.osm_id): p for p in existing_police if p.osm_id is not None}

        for elem in police_elements:
            osm_id = elem.get("id")
            tags = elem.get("tags", {})
            name = tags.get("name")
            ...
            try:
                ...
                if osm_id in existing_map:
                    police_station = existing_map[osm_id]
                    police_station.name = name
                    police_station.geom = from_shape(geom_shape, srid=4326)
                    updated += 1
                else:
                    new_police = PoliceStation(
                        osm_id=osm_id,
                        name=name,
                ...
                errors += 1
                logger.error(f"Error processing OSM police element {osm_id}: {fe}")
                continue
