...
from backend.app.models import Ward, Hospital
...
def parse_beds(tags):
    """
    Safely parses hospital beds tag count into an integer.
    Checks tags 'beds' and 'healthcare:beds'.
    """
    beds_val = tags.get("beds") or tags.get("healthcare:beds")
    if beds_val is None:
        return None
    if isinstance(beds_val, int):
        return beds_val
    val_str = str(beds_val).strip()
...
def ingest_hospitals(json_path: str):
    """
    Ingests healthcare facilities from raw OSM JSON into the database.
    Performs boundary filtering, validation, centroid normalization, and idempotent bulk upsert.
    """
    logger.info("Hospital ingestion started...")
    ...
    try:
        ...
        # Query all existing hospital osm_ids
        existing_hospitals = db.query(Hospital).all()
        existing_map = {int(h.osm_id): h for h in existing_hospitals if h.osm_id is not None}
        ...
        for elem in elements:
            ...
            try:
                ...
                accepted += 1
                beds = parse_beds(tags)

                # Perform Upsert
                if osm_id in existing_map:
                    hospital = existing_map[osm_id]
                    hospital.name = name
                    hospital.beds = beds
                    hospital.geom = from_shape(geom_shape, srid=4326)
                    updated += 1
                else:
                ...
                errors += 1
                logger.error(f"Error processing OSM hospital element {osm_id}: {fe}")
                continue
    ...
    logger.info("Hospital ingestion completed.")

if __name__ == "__main__":
    raw_file = os.path.join(project_root, "data", "raw", "infrastructure", "bhubaneswar_healthcare.json")
    ingest_hospitals(raw_file)
