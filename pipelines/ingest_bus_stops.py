...
from backend.app.models import Ward, BusStop
...
logger = logging.getLogger("bus_stops_ingestion")
...
def ingest_bus_stops(json_path: str):
    """
    Ingests bus stops from raw OSM JSON into the database.
    Performs boundary filtering, validation, centroid normalization, and idempotent bulk upsert.
    """
    logger.info("Bus stops ingestion started...")
    ...
    try:
        ...
        # Query all existing bus stop osm_ids
        existing_bus_stops = db.query(BusStop).all()
        existing_map = {int(b.osm_id): b for b in existing_bus_stops if b.osm_id is not None}
        ...
        for elem in elements:
            ...
            try:
                ...
                if osm_id in existing_map:
                    bus_stop = existing_map[osm_id]
                    bus_stop.name = name
                    bus_stop.geom = from_shape(geom_shape, srid=4326)
                    updated += 1
                else:
                ...
                errors += 1
                logger.error(f"Error processing OSM bus stop element {osm_id}: {fe}")
                continue
    ...
    logger.info("Bus stops ingestion completed.")

if __name__ == "__main__":
    raw_file = os.path.join(project_root, "data", "raw", "infrastructure", "bhubaneswar_bus_stops.json")
    ingest_bus_stops(raw_file)
